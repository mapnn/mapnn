...
DECLARE_OPTIMAL_MAP(map_ConvolutionTiledExecutorBasic2);

inline bool map_ConvolutionTiledExecutorBasic2::request(Operator& op) {
...
inline bool map_ConvolutionTiledExecutorBasic2::run(Graph* graph, Node* node) {
    std::string nodename = node->name();
    Operator op = node->getOp();
    node->setKernel(new mnn_ConvolutionTiledExecutorBasic2());

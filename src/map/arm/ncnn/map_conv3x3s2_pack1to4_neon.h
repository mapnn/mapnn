...
DECLARE_OPTIMAL_MAP(map_conv3x3s2_pack1to4_neon);

namespace mapnn {
inline bool map_conv3x3s2_pack1to4_neon::request(Operator& op) {
...
inline bool map_conv3x3s2_pack1to4_neon::run(Graph* graph, Node* node) {
    ...
    node->setKernel(new ncnn_conv3x3s2_pack1to4_neon());

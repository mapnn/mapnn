...
DECLARE_OPTIMAL_MAP(map_conv3x3s1_winograd64_neon4);


inline bool map_conv3x3s1_winograd64_neon4::request(Operator& op) {
...
inline bool map_conv3x3s1_winograd64_neon4::run(Graph* graph, Node* node) {
    ...
    GgG->setKernel(new ncnn_conv3x3s1_winograd64_transform_kernel_neon());
    node->setKernel(new ncnn_conv3x3s1_winograd64_neon4());

...
DECLARE_OPTIMAL_MAP(map_conv1x1s2_pack4_neon);

inline bool map_conv1x1s2_pack4_neon::request(Operator& op) {
...
inline bool map_conv1x1s2_pack4_neon::run(Graph* graph, Node* node) {
    std::string nodename = node->name();
    Operator op = node->getOp();
    Node* w_pack = graph->createNode(nodename+"_w_", op);
    Node* i_pack = graph->createNode(nodename+"_i_", op);
    Node* s2 = graph->createNode(nodename+"_s_", op);
    ...
    s2->setKernel(new ncnn_conv1x1s2_pack4_neon());

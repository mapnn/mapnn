...
inline bool map_conv3x3s1_pack1to4_neon::run(Graph* graph, Node* node) {
    std::string nodename = node->name();
    Operator op = node->getOp();
    Node* w_pack = graph->createNode(nodename+"_weight1to4_", op);
    ...
    node->setKernel(new ncnn_conv3x3s1_pack1to4_neon());

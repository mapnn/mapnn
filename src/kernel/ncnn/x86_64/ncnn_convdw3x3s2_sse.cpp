...
#include <layer/x86/convdw3x3s2_sse.h>
void ncnn_convdw3x3s2_sse::run(const Tensors& ins, Tensor& out, Tensors& tmp, Operator& op) {
    L1CHW output(out); 
    L1CHW input(ins[0]); 
    L111W weight(ins[1]); 
    L111W bias(ins[2]);
    ...
    ncnn::Option opt;
    ncnn::convdw3x3s2_sse(bottom_blob, top_blob, kernel, _bias, opt);
}

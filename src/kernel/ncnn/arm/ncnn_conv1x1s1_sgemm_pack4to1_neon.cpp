...
#include <layer/arm/conv1x1s1_sgemm_pack4to1_neon_interleave.h>
#include <layer/arm/conv1x1s1_sgemm_pack4to1_neon_sgemm.h>
void ncnn_conv1x1s1_sgemm_pack4to1_neon::init(const Tensors& ins, Tensor& out, Tensors& tmp, Operator& op) {
    Conv conv(op);
    LCHW4 input(ins[0]);
    L1CHW output(out);
    LUVA4 temp0(tmp[0]);
...
#if __aarch64__
    temp0.u = size/12 + (size%12)/8 + (size%12%8)/4 + size%12%4;
    temp0.v = conv.inch/4;
    temp0.a4 = 12*4;
#else
...
void ncnn_conv1x1s1_sgemm_pack4to1_neon::run(const Tensors& ins, Tensor& out, Tensors& tmp, Operator& op) {
    Conv conv(op);
    L1CHW output(out); 
    LCHW4 input(ins[0]); 
    LUVAB weight(ins[1]); 
...

    {
        ...
        ncnn::Option opt;
        ncnn::conv1x1s1_sgemm_pack4to1_neon_interleave(bottom_blob, top_blob, opt, output.c);
    }
    {
        ...
        ncnn::Option opt;
        ncnn::conv1x1s1_sgemm_pack4to1_neon_sgemm(bottom_blob, top_blob, kernel, _bias, opt, input.c, output.w, output.h, output.c);
    }
}

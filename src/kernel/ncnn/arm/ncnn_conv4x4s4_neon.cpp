...
#include <layer/arm/conv4x4s4_neon.h>
namespace mapnn {
void ncnn_conv4x4s4_neon::run(const Tensors& ins, Tensor& out, Tensors& tmp, Operator& op) {
    L1CHW input(ins[0]); 
    L1CHW output(out); 
    L111W weight(ins[1]); 
    L111W bias(ins[2]);
    ...
    ncnn::Option opt;
    ncnn::conv4x4s4_neon(bottom_blob, top_blob, kernel, _bias, opt);
}
}

...
#include <layer/arm/convdw3x3s1_pack4_neon.h>
namespace mapnn {
void ncnn_convdw3x3s1_pack4_neon::init(const Tensors& ins, Tensor& out, Tensors& tmp, Operator& op) {
    Conv conv(op);
    LCHW4 input(ins[0]); 
    LCHW4 output(out); 
...
void ncnn_convdw3x3s1_pack4_neon::run(const Tensors& ins, Tensor& out, Tensors& tmp, Operator& op) {
    LCHW4 output(out); 
    LCHW4 input(ins[0]); 
    LUVAB weight(ins[1]); 
    L111W bias(ins[2]);
    ...
    ncnn::Option opt;
    ncnn::convdw3x3s1_pack4_neon(bottom_blob, top_blob, kernel, _bias, opt);
}
}

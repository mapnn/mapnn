...
void mnn_transformWeightF63::run(const Tensors& ins, Tensor& out, Tensors& tmp, Operator& op) {
    Conv conv(op);
    LUVAB output(out);
    L111W input(ins[0]); 
    ...
    const float GT[] = {
        ...
        0.0f,  0.5f, -0.5f,  1.0f, -1.0f,  1.5f,  -1.5f,  0.0f,
        0.0f, 0.25f,  0.25f, 1.0f,  1.0f, 2.25f,  2.25f,  1.0f,
    };
    int ci          = conv.inch;
    int co          = conv.outch;
    ...
    for (int oz = 0; oz < co; ++oz) {
        ...
        for (int sz = 0; sz < ci; ++sz) {
            ...
            // M = G * K
            Math::Matrix::multi(M, G, srcSz, 8, 3, 3);
            // K_Transform = M*GT
            Math::Matrix::multi(KT, M, GT, 8, 3, 8);

...
class depthwise_test : public test {
private:
    std::string output = "perf_depthwise.txt";
    bool enable = true; 
    int cycle = 1;
    int kernel = 3;
    int stride = 1;
    int gap = 0;
public:
    depthwise_test(int argc, char** argv);
    int run()override;
};

depthwise_test::depthwise_test(int argc, char** argv) {
    int opt;
    const char *optstring = "k:s:c:o:g:h";
...
int depthwise_test::run() {
    ...
    for(int inh = 8; inh < 256; inh+=2) {
        ...
        for(int inch= 4; inch < 256; inch+=2) {
            int outch = inch+gap;
            if(2*inh*inh+inch*inch+outch*outch > 30000) continue;
            ...
            for(size_t p = 0; p < perfs.size(); p++) {
                fprintf(fp,"%d,%d,%d,%d,%d,%s,%6.5f,%6.5f,%6.5f\n", 
                        kernel, stride, inh, inch, (int)perfs.size(),

...
private:
    std::string output = "perf_conv.txt";
    bool enable = true; 
    int cycle = 1;
    int kernel = 3;
    int stride = 1;
    int gap = 0;
public:
    conv_test(int argc, char** argv);
    int run()override;
};

conv_test::conv_test(int argc, char** argv) {
    int opt;
    const char *optstring = "k:s:c:g:o:h";
    ...
    for(int inh = 8; inh < 256; inh+=2) {
        ...
        for(int inch= 4; inch < 256; inch+=2) {
            ...
            for(size_t p = 0; p < perfs.size(); p++) {
                fprintf(fp,"%-2d,%-2d,%-3d,%-3d,%-2d,%-40s,%-7f,%-7f,%-6.5f\n", 

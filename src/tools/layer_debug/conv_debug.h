...
class conv_debug : public layer_debug{
private:
    bool enable = true; 
    int kernel = 3;
    int stride = 1;
    ...
    int width = 4;
public:
    conv_debug(int argc, char** argv);
    int run()override;
};

conv_debug::conv_debug(int argc, char** argv) {
    int opt;
...
int conv_debug::run() {
    ...
    Tensor weight_tensor(outch, inch, kernel, kernel, FLOAT);
